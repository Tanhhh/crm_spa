...
namespace Erp.BackOffice.Administration.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public UserController(IUserRepository userRepo,
            ...
            ,IUserHistoryRepository userHistory
            )
        {
                    ...
                    if (user != null)
                    {
                        ...
                        if (Request.Files["file-image"] != null)
                        {
                            ...
                            if (file.ContentLength > 0)
                            {
                                ...
                                string image_name = "user_" + Helpers.Common.ChuyenThanhKhongDau(Regex.Replace(user.UserCode==null? user.UserName: user.UserCode, @"\s+", "_")) + "." + file.FileName.Split('.').Last();
                        ...
                        //ghi log 
                        Erp.BackOffice.Controllers.HomeController.WriteLog(model.Id,model.UserName,"đã cập nhật người dùng", "User/EditUser?userId=" + model.Id, Helpers.Common.CurrentUser.BranchId.Value);
            ...
            if (ModelState.IsValid)
            {
                if (!CheckUsernameExists(model.UserName.ToLower()))
                {
                    ...
                    try
                    {
                        ...
                        _userRepository.UpdateUser(user);
                        //ghi log 
                        Erp.BackOffice.Controllers.HomeController.WriteLog(user.Id, model.UserName, "đã tạo người dùng", "User/EditUser?userId=" + user.Id, Helpers.Common.CurrentUser.BranchId.Value);
        ...
        #region UserHistory
        public ViewResult UserHistory(int? userId, string startDate, string endDate)
        {
            ...
            DateTime d_endDate;
            var q = UserHistoryRepository.GetAllUserHistorys();
            ...
            IEnumerable<UserHistoryViewModel> model = q.Select(x => new UserHistoryViewModel
            {
                Id = x.Id,
            ...
            {
                model = model.Where(item => item.UserId == userId);
