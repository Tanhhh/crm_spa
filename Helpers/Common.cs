...
namespace Erp.BackOffice.Helpers
{
    ...
    public class Common
    {
		...
		public static void SendErrorToText(string errorStr)
		{
			try
			{
				...
				string filepath = HttpContext.Current.Server.MapPath("~/Logs/");  //Text File Path

				filepath = filepath + "LoggingException.txt";   //Text File Name
        ...
        public static string KiemTraTonTaiHinhAnhHoapd(string Image, string NameUrlImage, string NoImage, string ImagePath,string TargetModule = null)
        {
            //NameUrlImage là cột code trong setting .
            var ImageUrl = "";
            //chọn thay thế ảnh khi không có tên hình trong database.
        ...
        public static bool ResetCacheYHLALL()
        {
            try
            {
                string host = "";
                if (GetSetting("UrlAPI") != null)
                    host = GetSetting("UrlAPI").ToString();
                else
                    host = "";
                var client1 = new RestClient(host + "api/Cart/ResetCacheYHLALL");
                var k1 = new RestRequest(Method.POST);
                k1.AddHeader("cache-control", "no-cache");
                k1.AddHeader("Content-Type", "application/json; charset=utf-8");
                var response = client1.Execute(k1);
        ...
        public static bool ResetCacheYHL(string arrstr)
        {
            try
            {
                string host = "";
                if (GetSetting("UrlAPI") != null)
                    ...
                    host = "";
                var client1 = new RestClient(host + "api/Cart/ResetCacheYHL");
                var k1 = new RestRequest(Method.POST);
                k1.AddHeader("cache-control", "no-cache");
                k1.AddHeader("Content-Type", "application/json; charset=utf-8");

                k1.RequestFormat = DataFormat.Json;

                k1.AddJsonBody(new ModelKeyCache
                {
                    arrstr = arrstr
                });
                var response = client1.Execute(k1);
