...
namespace Erp.BackOffice.Helpers
{
    ...
    public static class HtmlHelperExtensions
    {
        ...
        public static MvcHtmlString MonthPicker<TModel, TProperty>(this HtmlHelper<TModel> htmlHelper, bool IsCheckValidation, Expression<Func<TModel, TProperty>> expression, string labelClass = "control-label col-lg-5 col-md-4 col-sm-4", string controlClass = "col-lg-7 col-md-8 col-sm-8", string inputClass = "col-sm-12")
        {
            var result = ((LambdaExpression)expression).Compile().DynamicInvoke(htmlHelper.ViewData.Model);
            var fullHtmlFieldName = ExpressionHelper.GetExpressionText(expression);
        ...
        public static MvcHtmlString YearPicker<TModel, TProperty>(this HtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TProperty>> expression, int before = 5, int after = 5, string labelClass = "control-label col-lg-5 col-md-4 col-sm-4", string controlClass = "col-lg-7 col-md-8 col-sm-8", string inputClass = "col-sm-12")
        {
            var result = ((LambdaExpression)expression).Compile().DynamicInvoke(htmlHelper.ViewData.Model);
            var titleUI = htmlHelper.LabelFor(expression, new { @class = "control-label no-padding-right " + labelClass });
            var fullHtmlFieldName = ExpressionHelper.GetExpressionText(expression);
            ...
            {
                inputUI = htmlHelper.DropDownListFor(expression, SelectListHelper.GetSelectList_Number(DateTime.Now.Year - before, DateTime.Now.Year + after, result).Where(item => !string.IsNullOrEmpty(item.Value)), new { style = "width:70px" });
            }
            else
            {
                inputUI = htmlHelper.DropDownListFor(expression, SelectListHelper.GetSelectList_Number(DateTime.Now.Year - before, DateTime.Now.Year + after, result).Where(item => !string.IsNullOrEmpty(item.Value)), new { style = "width:70px" });
        ...
        public static MvcHtmlString YearInput<TModel, TProperty>(this HtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TProperty>> expression, bool IsCheckValidation, string labelClass = "control-label col-lg-5 col-md-4 col-sm-4", string controlClass = "col-lg-7 col-md-8 col-sm-8", string inputClass = "col-sm-12")
        {
            var result = ((LambdaExpression)expression).Compile().DynamicInvoke(htmlHelper.ViewData.Model);
            var titleUI = htmlHelper.LabelFor(expression, new { @class = "control-label no-padding-right " + labelClass });
            var fullHtmlFieldName = ExpressionHelper.GetExpressionText(expression);
            ...
            MvcHtmlString inputUI = htmlHelper.TextBoxFor(expression, new { style = "width:70px", type = "text" });
